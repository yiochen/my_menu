...
extension ProcessingCoordinatorCovers on ProcessingCoordinator {
  Future<void> processPendingCovers() async {
    ...
    for (final ProcessingOutboxRequest request in requests) {
      ...
      if (request.deliveryState == ProcessingDeliveryState.acknowledged) {
        if (request.adoptionState == ProcessingAdoptionState.readyForAdoption) {
    ...
    try {
      if (request.deliveryState == ProcessingDeliveryState.pendingUpload ||
        ...
        final ApiProcessingJob job = await _processingApi
            .createProcessingJob(
              contract: ApiProcessingContract.coverGenerationV1,
        ...
        await _processingApi
            .submitProcessingJob(
              jobId: job.id,
              input: ApiCoverGenerationInput(_coverProcessingInput(request)),
            )
            .timeout(_controlRequestTimeout);
      ...
      final ApiProcessingResult downloaded = await _processingApi
          ...
          .timeout(_controlRequestTimeout);
      if (downloaded is! ApiCoverGenerationResult) {
      ...
      if (existing == null) {
        if (!await _coverSourcesStillBelongToDish(request)) {
          await _processingApi.cancelProcessingJob(jobId: job.id);
          await outbox.markFailed(
            request.id,

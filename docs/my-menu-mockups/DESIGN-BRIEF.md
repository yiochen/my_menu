...
- Dish deletion: a menu-level, multi-select mutation that removes each selected dish and its dependent MyMenu history, notes, source copies, and planned meals. Ideas and other dishes with no cooking history are still eligible; background photo-processing records are never dishes or selection targets. Deletion never changes originals in the phone photo library and remains briefly recoverable as one atomic Undo.
...
- Captured photo: durable local source media that is immediately browsable whether or not processing is available. Its organization state is derived from its current dish association, independently of background processing.
- Photo organization: a local, reversible association between a captured photo and a dish. Explicit user choices outrank heuristics and late AI proposals.
- Processing request: hidden background delivery and proposal work. It can organize or enrich a captured photo, but it is never the source of the user-facing Photos collection.
...
Plan and Menu retain the same floating bottom shell: Plan first, centered global capture action, Menu second. Dishes is the default Menu surface. Its compact fixed search row is shortened to reserve one trailing Photos button, which opens the dedicated image gallery without adding a permanent Photos, Capture, Inbox, or Review tab. The button is neutral when all saved photos are organized, shows a stationary orange unorganized count when manual attention is available, and uses a subtle orbit plus sparkle while AI organization is active; reduced-motion users see a static outlined state. Photos has its own compact back–title/count–Select row above All, Unorganized, and Organized filters. The fixed chrome gains a shadow only after vertical scrolling begins. Selection replaces the collection controls in place. Plan uses a fixed, horizontally pageable seven-day calendar and a horizontally pageable selected-day card; the month/year opens date selection and one Today control replaces week arrows. The orange filled action is reserved for the highest-frequency contextual task. Rows and photo tiles open their detail task; infrequent move/remove actions live in detail or overflow and remain reversible. Capture starts as a modal with one close path and no request to classify before capturing, while a completed created-dish result opens as a full screen.

Capture outcomes explicitly distinguish local save from AI organization. A locally saved photo appears immediately in the Photos gallery; background work is communicated with one quiet tile overlay rather than a temporary dish card. High-confidence organization can complete automatically with Undo, while ambiguity becomes Review and manual organization is always available. Offline capture remains fully viewable and assignable. Add Idea captures only the idea and an optional note; planning remains a separate Plan task. Improve Cover is a persistent chip on the cover image and never alters source photos; it progresses through source selection, optional freeform visual direction, generation, comparison, and explicit acceptance.
...
- Menu: typical/dense content is represented as one unified grid under a sticky shortened search utility bar with the Photos status button at right, controlled by All, Favorites, Recently added, and Filters. Neutral, unorganized-count, and animated-organizing button states are covered. Menu-level management covers long-press entry, multi-selection across the current filtered results, a replacement selection header, scoped destructive confirmation, updated counts, and Undo. Empty is represented separately because it materially changes the primary task.
- Photos: the typical All gallery, Unorganized and Organized filters, dense/long content, empty content, missing media, offline/background work, Review, and failed organization are required. Photos use a three-column date-grouped grid with one status overlay per tile. Tile detail requires canonical unorganized, organizing, review, and organized states, plus assignment, change, overflow, destructive confirmation, completion, and Undo paths.
...
- Menu status details: captures needing attention appear as a stationary count on the Photos entry plus restrained gallery overlays and an Unorganized filter count; active AI organization animates only the Photos entry and relevant tile, then settles to the count or neutral state. Newly created dishes use a styled white “New” label that gives one brief wobble as the card appears.

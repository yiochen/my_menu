...
`finalize-capture-batch` is the app-facing scheduler for capture organization.
It should durably queue a `batch_grouping` AI job, enqueue `process-ai-jobs`
through the database `pg_net` RPC, and return without waiting for AI work. Keep
`process-ai-jobs` as an internal worker guarded by `x-mymenu-worker-key`.
